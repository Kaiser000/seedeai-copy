...
并附上完整固定词表（15 分类 + 12 情绪 + 3 格式），强制 LLM 从词表精确选择。情绪词表从原始 6 个扩展到 12 个（新增 复古怀旧 / 赛博未来 / 童趣可爱 / 自然有机 / 严肃纪实 / 节庆狂欢），同时 gene 对象新增 `layoutStyle`（6 种构图风格）和 `colorStrategy`（4 种配色策略）字段。完整词表见当前 `poster-analyze.md` 文件或 [tech-plan.md 第 4.3 节](./tech-plan.md#templatehint-固定词表)。
...
排序后取扩大的 top pool（至少 `count*8`，最多 20 条），shuffle 后通过 `selectWithCategoryDiversity()` 抽 `count` 条返回，优先保证不同 category。

**多样性三重保障**：随机扰动（打破得分接近的固定排序）+ 扩大池子（count×8=16）+ 分类去重（selectWithCategoryDiversity 两轮选择）。

#### Step 3：把 format 三分类与画布尺寸对齐

```java
private String classifyFormat(int width, int height) {
    if (height <= 0) return "长图";    // 自适应长图（height=0 哨兵值）
...
**自适应长图模式**：前端传 `height=0` 表示自适应，后端/模板检索统一归为"长图"格式。
...
3. 解析 ```json 块，提取并写入：
   ...
   - `gene.layoutStyle` → 写入"构图风格"（6 种之一，驱动布局技术选择）
   - `gene.colorStrategy` → 写入"配色策略"（4 种之一，驱动色彩推导方法）
...
#### Step 7：实现样本骨架提取（两阶段：文案压缩 + 板式裁剪）

**核心理念：结构全盘保留，文案压缩，装饰性板式可挑选。** "骨架"不是"前 N 个字符"，而是剥去了血肉（文案）和装饰（板式细节）的结构蓝图。

**Phase 1 — 文案压缩**（6 个步骤，使用预编译的 `PAT_*` 正则常量）：
1. 去除 import / ReactDOM 样板代码
2. 压缩标签间长中文文本：`>五一国际劳动节快乐<` → `>五一国际...<`
3. 压缩 img prompt 属性（长描述 → 前 10 字符）
4. 压缩含中文的引号字符串（数据数组中的标题/描述）
5. 深层缩进减半（保留嵌套层级，省空间）
6. 折叠连续空行

**Phase 2 — 板式裁剪**（仅在 Phase 1 后仍超 12000 字符预算时执行）：
1. 移除装饰性 className（shadow-/rounded-/border-/opacity-/transition/hover:）
2. 移除 textShadow 样式
3. 缩写超长 className 链（保留前 60 字符布局类）

**兜底**：若两阶段后仍超预算，对已高度压缩的代码做头部截取。

实测效果：324 个模板中 **74.4% 完整保留**，中位数模板（12661 chars）压缩后 ~9900 chars 完整通过。预算从旧版 3500 放宽到 12000（2 样本 × 12K = 24K chars ≈ 8K tokens，占 128K 上下文仅 6%）。
...
frontend/src/engine/handlers/textHandler.ts        ← 单段 + 混合样式 Textbox
...
frontend/src/engine/parsers/inlineTextParser.ts    ← 内联文本容器识别与段收集
...
3. 根据节点类型分发到 handler：
   ...
   - 纯文本叶节点 → `textHandler.createTextObject` → `fabric.Textbox`
   - **内联文本容器**（所有子节点是 TEXT_NODE 或纯文本 `span`/`b`/`em` 等，≥2 段）
     → `inlineTextParser.collectInlineSegments` + `textHandler.createStyledTextObject`
     → 带 per-character `styles` 的单个 `fabric.Textbox`，保留各段不同字号/颜色并自动基线对齐
...
#### Step 5：渲染失败时的画布状态诚实化

LLM 生成的 JSX 偶尔会出现变量名 typo（如声明 `typography` 却写 `typographic.body`）。这类 `ReferenceError` 在 `renderToHiddenDom` 的 `flushSync` 阶段同步抛出，如果 catch 只 `setError` 而不清画布，上一次成功渲染的 fabric 对象会继续显示，用户会误以为"图片/某个区块丢了"，实际整棵组件都没渲染成功。

复现要点：[CanvasPanel.tsx](../frontend/src/features/editor/components/CanvasPanel.tsx) 的 `runRender` catch 块必须同时做：

```typescript
} catch (err) {
  const rawMessage = (err as Error).message || 'unknown error'
  console.error('[CanvasPanel] Render pipeline failed:', { error: rawMessage, ... })

  // 1. 清空画布：让空白画面诚实反映"渲染失败"
  ...
  // 2. 把真实错误类型透给用户（ReferenceError / SyntaxError / TypeError 均可见）
  setError(`渲染失败：${rawMessage}`)
}
```

错误条的 UI 要允许较长文案（`max-w-[260px]` + `title={error}` tooltip），确保至少能在 hover 时看到完整信息。这样当 LLM 再次出现 typo 时，用户能一眼看到 `渲染失败：typographic is not defined`，立刻知道去 chat 里要 LLM 改正变量名，而不是误判成图片/网络问题。

对应的预防措施在 [poster-generate.md](../backend/src/main/resources/prompts/poster-generate.md) 的「守则 6：编译安全」里，加了"变量名严格一致性"硬约束，要求 LLM 所有 `{variable}` 引用必须与 `const` 声明逐字符一致。

#### Step 6：几何审计 + 自动修复闭环

JSX 路线无法做解码阶段的 grammar-constrained decoding，所以稳定性必须靠"渲染后审计 + LLM 回注修复"兜底。完整实现见 [tech-plan § 8.5](./tech-plan.md#85-几何审计geometric-audit)，这里只讲复现步骤。

**要做的事**：在 `runRender` 中，`renderToHiddenDom` + 图片等待完成之后、`convertDomToCanvas` 之前，插入一次 `runGeometricAudit(hiddenDiv, posterSize)` 调用，把结果写入 `useEditorStore.auditReport`。

**四个落地文件**：

1. [`engine/audit/auditTypes.ts`](../frontend/src/engine/audit/auditTypes.ts) —— 类型定义，`AuditIssue` / `AuditReport` / `AuditOptions`，所有阈值都可注入。
2. [`engine/audit/auditHelpers.ts`](../frontend/src/engine/audit/auditHelpers.ts) —— 纯函数工具。务必把 `parseCssColor`、`relativeLuminance`、`contrastRatio`、`compositeOver`、`computeBoundsOverflow` 做成纯函数，jsdom 不实现布局引擎，只有纯函数可以在测试里精确断言。
3. [`engine/audit/geometricAudit.ts`](../frontend/src/engine/audit/geometricAudit.ts) —— 主函数 `runGeometricAudit(root, bounds, options?)`。深度优先遍历 root 的后代，对每个元素跑四条规则：
   - `OUT_OF_BOUNDS`：`el.getBoundingClientRect()` 相对 root 偏移量 → `computeBoundsOverflow`；要先排除 `overflow:hidden` 祖先（内容被裁切）和"视觉不显著"的纯布局容器（无背景/边框/阴影），否则误报爆炸。
   - `TEXT_OVERFLOW`：文本叶节点的 `scrollWidth > clientWidth + 2` 或 `scrollHeight > clientHeight + 2`。
   - `MIN_FONT_SIZE`：文本叶节点 computed `font-size < 14px`（可配）。
   - `LOW_CONTRAST`：文本叶节点 `color` vs 递归向上找到的"有效背景色"（遇到半透明用 `compositeOver` 合成），WCAG 对比度 < 4.5。
   - **整个函数必须裹一层 try/catch**：审计崩溃绝不能阻塞渲染主流程。
4. [`engine/audit/auditFormatter.ts`](../frontend/src/engine/audit/auditFormatter.ts) —— `formatIssuesForRepair(report)` 把 `AuditReport` 格式化成一段中文 LLM 修复 prompt（分"必须修复/建议修复"两段，末尾加"保持整体设计不变、只做最小改动、输出完整 JSX"三条修复约束）；`formatIssuesForUi(report)` 格式化成 Banner 用的精简条目。

**修复闭环**（核心：不改后端，复用 `/api/posters/chat`）：

```typescript
// useAuditRepair.ts
const repairPrompt = formatIssuesForRepair(auditReport);  // 中文结构化指令
await connectSse('/api/posters/chat', {
  canvasState: serializeCanvas(canvas),
  userMessage: repairPrompt,          // ← 把审计结果直接当成用户输入
  chatHistory: ...,
  width, height, modelName,
}, { onError: ... });
// 拿到新 JSX → setGeneratedCode() → CanvasPanel 自动重渲染 → 自动重审计
```

**UI 展示**：[`AuditBanner.tsx`](../frontend/src/features/editor/components/AuditBanner.tsx) 是 CanvasPanel 右上角浮层，只在 `errorCount > 0 || warningCount > 0` 时显示，能展开查看 issue 列表，带"自动修复"按钮（调 `useAuditRepair.repair()`）和"忽略"按钮（清空 auditReport）。生成中 (`isGenerating`) 时自动隐藏避免闪烁。

**测试策略**：jsdom 没有布局引擎，`getBoundingClientRect` / `scrollWidth` 全是 0。对策：

- **纯函数** 在 [`__tests__/engine/auditHelpers.test.ts`](../frontend/src/__tests__/engine/auditHelpers.test.ts) 精确断言（WCAG 21:1 对比度、线性 alpha compositing、rgb/hex 解析所有分支）。
- **主函数** 在 [`__tests__/engine/geometricAudit.test.ts`](../frontend/src/__tests__/engine/geometricAudit.test.ts) 用一个 `mockLayout(el, {left,top,width,height,scrollWidth,clientWidth,...})` 工具函数打补丁，覆盖了 OUT_OF_BOUNDS 触发/被 overflow:hidden 抑制/视觉不显著时不报、TEXT_OVERFLOW、MIN_FONT_SIZE 阈值可配等 11 个场景。

**设计原则**：

- **宁可漏报，不要误报**。误报会把 LLM 带跑偏，生成质量会比没有 audit 还差。所以元素重叠（布局意图不明）、VLM 审美判分（延迟/成本）等规则先不做。
- **issue.message 面向 LLM**：直接写中文修复建议（"请缩短文本或允许换行"），而不是机器可读的规则 ID。
- **`setAuditReport(null)` 触发修复时立即清空**，让 UI 在新一轮审计跑完前进入"已触发修复"中间态，避免旧 banner 闪烁。
...
**驱动力**：与竞品对比发现 5 个核心差距——内容丰富度不足、样式变化少、背景缺乏高级感、缺少圆角形状设计、不根据场景选择合适布局。

**改动概览**：

1. **poster-analyze.md**：新增「场景适配内容架构」映射表（7 种场景 × 必选组件 × 最低区块数），强制 LLM 根据场景规划特定组件（如活动→时间轴、旅游→路线卡片、电商→商品网格）。新增「背景高级感规范」和「圆角容器规范」。最低区块数从 3/4-5/6-8 提升到 4/6/8。

2. **poster-generate.md**：新增守则 7（背景高级感，禁止纯色平铺，提供 4 种高级手法代码模式）和守则 8（圆角容器系统，强制所有卡片/按钮/标签使用圆角）。新增「场景适配组件库」（时间轴、编号路线卡、商品网格、数据展示、引用框共 5 种代码级模式）。负面提示扩展到 14 条。

3. **design-reference.md**：新增模式 6-9（径向辉光背景、时间轴流程、编号路线卡片、圆角容器系统）。

4. **technique-snippets.json**：新增 6 种手法（radial-glow-background、section-bg-alternation、timeline-flow、numbered-route-card、warm-white-background）。

5. **TemplateService.getTechniqueSnippets**：改为双池策略——2 条来自必选池（标签含 background/premium/layout/timeline 的手法），其余来自高频池。手法注入数从 6 提升到 8。
...
| 背景全红/全蓝，缺乏高级感 | LLM 使用纯色平铺而非渐变 | 守则 7（背景高级感）已禁止纯色平铺，提供了径向辉光/多层渐变/深浅交替等手法。检查生成 JSX 中的 `backgroundColor` 是否仍是单一纯色 |
| 内容少、区块结构单一 | LLM 未按场景选择合适组件 | poster-analyze.md 新增了「场景适配内容架构」映射表，检查分析阶段 JSON 的 sections 数量是否满足最低区块数要求 |
| 卡片/容器是直角矩形 | LLM 未使用圆角 | 守则 8（圆角容器系统）已强制所有容器有圆角，检查生成 JSX 中的 div 是否有 `rounded-*` 类名 |
