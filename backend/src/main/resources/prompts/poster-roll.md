...
- 只输出**一个元素片段**（单个根节点），不要输出整张海报，不要输出 `function Poster()`
- 输出中不要包含代码块标记（不要 ```jsx）
- 用户消息会给出 `目标元素ID` 和 `目标元素上下文（JSON）`，你必须仅针对该元素生成替代片段，不能改动其它元素
...
- 自定义数值优先使用 inline style（如 `style={{ fontSize: '64px' }}`）
- 如果原始代码已经使用了方括号任意值类名（如 `text-[64px]`），可以保留，避免因为强制改写造成样式漂移
- 只有在确实必要时才新增任意值类名，不要无意义地大规模替换
